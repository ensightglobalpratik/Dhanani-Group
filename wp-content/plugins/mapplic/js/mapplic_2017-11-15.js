            ...
            this.init = function() {
                ...
                if (self.o.search) {
                    ...
                    var input = $('<input>').attr({
                        'type': 'text',
                        'spellcheck': 'false',
                        'placeholder': mapplic_localization.search
                    ...
                    $.each(level.locations, function(index, location) {
                        ...
                        if (location.pin.indexOf('hidden') == -1) {
                            if (self.o.markers) {
                                ...
                                pin.attr('data-title', location.title);
...
jQuery(document).ready(function() {
    ...
    $("<select class='map-select'></select>").insertBefore($(".mapplic-element"));
    $("[data-location]").each(function(ind, elem) {
        var loc = $(elem).attr('data-location');
        var title = $(elem).attr('data-title');
        $(".map-select").append('<option value="' + loc + '" data-value="' + window.location.href.split("?")[0] + '?location=' + loc + '" >' + title + '</option>')
    });
    var link = window.location.href;
    if($("[data-value='"+link+"']").length) {
        $(".map-select").val($("[data-value='"+link+"']").attr('value'));
    }
    $(".map-select").change(function(event) {
        window.location.href = $(this).find(':selected').attr("data-value");
    });

    /* old */
    $("body").on('click', '#prints', function() {
        var contents = $(".mapplic-tooltip-content").html();
        $("#myPDF#prints").hide();
        var frame1 = $('<iframe  />');
        ...
        var frameDoc = window.open('', '', 'height=700,width=1000');
        frameDoc.document.open();
        //Create a new HTML document.
        frameDoc.document.write('<html><head><title>DIV Contents</title>');
        frameDoc.document.write('<style>  strong { font-weight: 700; } table { width: 100%; }  table, th, td { border: 1px solid black; border-collapse: collapse; font-family: Helvetica,Arial,sans-serif; line-height: 1.2; padding: 10px 5px;-webkit-box-sizing: border-box;-moz-box-sizing: border-box;box-sizing: border-box;}.btn{background: #f0750f; display: inline-block; vertical-align: top; color: #fff !important; text-align: center; line-height: 1; font-weight: 900; text-transform: uppercase; padding: 10px 24px 10px; font-family: "Lato", sans-serif; cursor:pointer; margin-top:10px;} .btn:hover { background: #008af0; color: #fff;} </style>');
        frameDoc.document.write('</head><body>');
        frameDoc.document.write('<div id="myPDF" >');
        //Append the external CSS file.
        //Append the DIV contents.

        frameDoc.document.write('<div style="text-align:center;"><button class="btn btn-sm pull-right btn-default" id="print-btns">Print</button> <button class="btn btn-sm pull-right btn-default" onclick="javascript:demoFromHTML();">Save As PDF</button> <button class="btn btn-sm pull-right btn-default pdf-btns">PDF</button> </div></body></html>');
        frameDoc.document.write(contents);
        frameDoc.document.write("</div>");
        frameDoc.document.write("<script src=\'https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js\'><\/script>");
        frameDoc.document.write('<script src=\"https://unpkg.com/jspdf@latest/dist/jspdf.min.js\"></script>');
        frameDoc.document.write('<script type="text/javascript">$(document).ready(function($) { $("#print-btns").click(function () {$("#print-btns").hide();  $(".pdf-btns").hide(); window.print(); });  });</script>');
        frameDoc.document.write('<script type="text/javascript"> function demoFromHTML() { alert(1); $("td:hidden,th:hidden","#myPDF").show();var pdf = new jsPDF("l", "pt", "a4");pdf.cellInitialize();pdf.setFontSize(12);$.each( $("#myPDF tr"), function (i, row){$.each( $(row).find("td, th"), function(j, cell){var txt = $(cell).text().trim().split(" ").join("\\n") || " "; var width = (j==0) ? 100 : 100; console.log(txt, txt.length); if(txt.length > 10) pdf.cell(10, 60, width+50, 100, txt, i); else  pdf.cell(10, 60, width, 60, txt, i);});});pdf.save("country-details.pdf");} </script>');
        //frameDoc.document.write('<script type="text/javascript">var doc = new jsPDF("landscape"); var specialElementHandlers = { "#editor": function (element, renderer) { return true; } }; $("body").on("click",".pdf-btns",function (){doc.fromHTML($("#myPDF").html(), 100, 100, { "width": "1750" , "elementHandlers": specialElementHandlers } ) ;  doc.save("sample-file.pdf"); });</script>');
        //frameDoc.document.write('<script type="text/javascript">var doc = new jsPDF(); var specialElementHandlers = { "#editor": function (element, renderer) { return true; } }; $("#pdf-btns").click(function () { margins = { top: 80, bottom: 60, left: 40, width: 522 }; doc.fromHTML($("table").html(), 15, 15, { "width": margins.width, "elementHandlers": specialElementHandlers }); doc.save("sample-file.pdf"); });function demoFromHTML() { var pdf = new jsPDF("landscape"); source = $("#myPDF")[0]; specialElementHandlers = { "#bypassme": function (element, renderer) { return true } }; margins = { top: 80, bottom: 60, left: 40, width: 522 }; pdf.fromHTML( source, margins.left, margins.top, { "width": margins.width, "elementHandlers": specialElementHandlers }, function (dispose) { pdf.save("Test.pdf"); }, margins); }</script>');
    ...
    /*$("body").on('click', '.pdf-btns', function() {
    	demoFromHTML();
    });
    function demoFromHTML() {    	 
        $("td:hidden,th:hidden","#pdf-print").show();
    	var pdf = new jsPDF('l', 'pt', 'a4');
     	pdf.cellInitialize();
	    pdf.setFontSize(10);
	    $.each( $('#pdf-print tr'), function (i, row){
	        $.each( $(row).find("td, th"), function(j, cell){
	        	$(this).css('width','100%');
        	 var txt = $(cell).text().trim().split(" ").join("\\n") || " ";
             var width = (j==0) ? 100 : 100; //make with column smaller
