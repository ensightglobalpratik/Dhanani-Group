            ...
            $.each(self.data.levels, function(index, level) {
                ...
                $.each(level.locations, function(index, location) {
                    if (location.id == id) {
                        ...
                        $(".mapplic-tooltip .mapplic-tooltip-description").scrollTop(0);
...
jQuery(document).ready(function() {
    ...
    $("<div class='container'><select class='map-select'></select></div>").insertBefore($(".mapplic-element"));
    $(".map-select").html("<option data-default=true >Choose a Country</option>");
    ...
    /* alphabetical sort script starts here */
    var mylist = $('.map-select');
    var listitems = mylist.children('option[data-value]').get();
    listitems.sort(function(a, b) {
        var compA = $(a).text().toUpperCase();
        var compB = $(b).text().toUpperCase();
        return (compA < compB) ? -1 : (compA > compB) ? 1 : 0;
    ...
    /* alphabetical sort script ends here */
    var link = window.location.href;
    if ($("[data-value='" + link + "']").length) {
        $(".map-select").val($("[data-value='" + link + "']").attr('value'));
        $("html, body").animate({
            scrollTop: $(".map-select").offset().top - $("header").outerHeight()
        }, 500);
    }
    $(".map-select").change(function(event) {
        if (!$(this).find(':selected').attr("data-default")) {
